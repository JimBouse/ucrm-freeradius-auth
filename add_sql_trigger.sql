...
DELIMITER //
CREATE TRIGGER `add_unknown_devices` AFTER UPDATE ON `radpostauth` 
FOR EACH ROW 
BEGIN
   IF NEW.reply = 'Access-Reject' THEN
     ...
     INSERT INTO radusergroup(username, groupname, priority) VALUES (OLD.username, 'Service_Unknown_Device', 1);
   END IF;
END//
DELIMITER ;
